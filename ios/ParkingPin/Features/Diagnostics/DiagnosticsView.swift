...
struct DiagnosticsView: View {
    ...
    private var drivingSessionSection: some View {
        Section("주행 세션 (bounded)") {
            ...
            // docs/04_IOS §3a. Read these together: a request with no start means the adapter
            // never ran; a start with no updates means Core Location said nothing; a start
            // in the background with updates arriving is the capture working as designed.
            ...
            LabeledContent("업데이트 중", value: model.snapshot.captureHealth.isUpdating ? "YES" : "NO")
            LabeledContent("업데이트 수신", value: "\(model.snapshot.captureHealth.updateCount)회")
            LabeledContent(
                "포그라운드 시작",
                value: model.snapshot.captureHealth.startedInForeground.map { $0 ? "YES" : "NO" } ?? "-"
            )
