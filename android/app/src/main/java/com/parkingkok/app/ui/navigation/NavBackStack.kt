 ...
 */
@JvmInline
value class NavBackStack private constructor(val entries: List<ParkingkokRoute>) {

    /** Where the user is. Never empty — [ParkingkokRoute.Home] is the floor of the stack. */
    val current: ParkingkokRoute get() = entries.last()
     ...
     * for.
     */
    fun push(route: ParkingkokRoute): NavBackStack =
     ...
     */
    fun replaceTop(route: ParkingkokRoute): NavBackStack =
        NavBackStack(entries.dropLast(1) + route)

    /** Unwinds to the root. */
    fun popToRoot(): NavBackStack = NavBackStack(listOf(entries.first()))

    internal fun encode(): List<String> = entries.map(ParkingkokRouteCodec::encode)
    ...
    companion object {
        ...
        internal fun decode(values: List<String>): NavBackStack {
            val routes = values.mapNotNull(ParkingkokRouteCodec::decode)
