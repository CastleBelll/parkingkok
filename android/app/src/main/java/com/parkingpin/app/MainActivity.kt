...
import com.parkingpin.app.detection.ParkingCandidateChannel
...
class MainActivity : ComponentActivity() {
     ...
     *
     * ```sh
     * adb shell am start -n com.parkingpin.app/.MainActivity --ez pk_firebase_selfcheck true
     * ```
     *
     * There is no backend feature yet, so nothing in the product calls
     * [com.parkingpin.app.identity.AnonymousIdentity]. This is how both the sign-in and the
     ...
     *
     * ```sh
     * adb shell am start -n com.parkingpin.app/.MainActivity \
     ...
     * [com.parkingpin.app.detection.ParkingDetectionRuntime] at compressed timestamps.
     *
     * **It bypasses nothing that decides.** The events go through the same reducer,
     * the same §3a table, the same §8 scoring and the same
     * [com.parkingpin.app.detection.ParkingCandidateCoordinator] a Play services transition
