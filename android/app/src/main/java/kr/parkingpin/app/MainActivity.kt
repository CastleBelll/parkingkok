...
import kr.parkingpin.app.detection.ParkingCandidateChannel
...
import kr.parkingpin.app.ui.navigation.ParkingpinApp
...
class MainActivity : ComponentActivity() {
     ...
     *
     * ```sh
     * adb shell am start -n kr.parkingpin.app/.MainActivity --ez pk_firebase_selfcheck true
     * ```
     *
     * There is no backend feature yet, so nothing in the product calls
     * [kr.parkingpin.app.identity.AnonymousIdentity]. This is how both the sign-in and the
     ...
     *
     * ```sh
     * adb shell am start -n kr.parkingpin.app/.MainActivity \
     ...
     * [kr.parkingpin.app.detection.ParkingDetectionRuntime] at compressed timestamps.
     *
     * **It bypasses nothing that decides.** The events go through the same reducer,
     * the same §3a table, the same §8 scoring and the same
     * [kr.parkingpin.app.detection.ParkingCandidateCoordinator] a Play services transition
